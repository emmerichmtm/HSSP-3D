...
\begin{table}[t]
\centering\footnotesize
\caption{Smallest $k$ for which the operation bound of the dynamic programme drops below the cost of brute
force ($k\le\min(n/2,3000)$ examined; ``--'': never).  ``default'': $c_b=1$, $c_\gamma=2.5$, $c_s=1$;
``proven'': the constants of Miller's theorem.}
\label{tab:crossover}
\begin{tabular}{r|rrr|rrr}
\toprule
 & \multicolumn{3}{c|}{default constants} & \multicolumn{3}{c}{proven constants}\\
$n$ & $k$ & DP $\le$ & BF & $k$ & DP $\le$ & BF\\
\midrule
...
where both methods need about $10^{360}$ operations.  With the proven constants no cross-over occurs for
$n\le 10^9$ and $k\le 3000$.  The advantage of the algorithm is thus real but purely asymptotic: it lies
entirely in a regime in which neither method can be executed.
